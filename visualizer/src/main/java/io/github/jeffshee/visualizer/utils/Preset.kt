...
class Preset {
    companion object {
        ...
        fun getPresetWithBitmap(name: String, bitmap: Bitmap, colors : List<Int>, size : Float): Painter {
            return when (name) {
                "cIcon" -> Sequential(listOf(Rotate(FftCircle()), SimpleIcon(SimpleIcon.getCircledBitmap(bitmap))))
                "cWaveRgbIcon" -> Sequential(
                    listOf(
                        Rotate(FftCircleWaveRgb(color = colors, radiusR = (size + 0.08f))),
                        SimpleIcon(SimpleIcon.getCircledBitmap(bitmap), size)
                    )
                )
